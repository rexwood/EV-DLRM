...
def copycatCacheReplacementPolicy(key):
    ...
    global EvTable
    return EvTable[int(tableNum)][int(idx), :]
###########################################EvLFU##########################################
cap = 768
min = 0
vals = dict()
...
nPerfectItem = 0
flushRate = 0.4
perfectItemCapacity = 1.0


def request(group_keys):
    aggHitMissRecord = []
    aggHit = 0
    global cap, min, vals, counts, lists, nPerfectItem, flushRate, perfectItemCapacity
...
#        print(V)
    ...
    if lists.get(26) and not len(lists.get(26)) == 0:
        nPerfectItem = len(lists.get(26))
...
def getValFromMem(key, aggHit):  # Get From Mem
    ...
    if count == min:
        while (lists.get(min) is None) or len(lists.get(min)) == 0:
            min += 1
    if lists.get(newCount) is None:
        lists[newCount] = []
        lists = dict(sorted(lists.items()))
    lists.get(newCount).append(key)
    return vals[key]
...
def set(key, value, aggHit):
    ...
    # Flushing:
    if nPerfectItem >= int(cap * perfectItemCapacity):
        # print("flushing!")
        for i in range(0, int(flushRate * cap) + 1):
            evictKey = lists.get(26)[0]
            lists.get(26).remove(evictKey)
            vals.pop(evictKey)
            counts.pop(evictKey)
    ...
    if aggHit >= min:
        ...
        if lists.get(aggHit) is None:
            lists[aggHit] = []
            lists = dict(sorted(lists.items()))
        lists.get(aggHit).append(key)
    else:
        min = aggHit
        if len(vals) < cap:
            vals[key] = value
...
#             print(sparse_index)
        ...
        _, ly = request(group_keys)
#             V1 = copycatCacheReplacementPolicy(key)
#             V = []
#             V.append(V1)
#             V = torch.tensor(V, dtype=torch.float32)
# #             print(V)
