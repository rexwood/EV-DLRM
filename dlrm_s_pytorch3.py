...
####################################################################################################
# EvLFU part
import EvLFU4DLRM_C1
import EvLFU4DLRM_C2
def mixPrecisionRequest(group_keys):
    # generate aggHit-C1:
    aggHitC1 = 0
    aggHitMissRecordC1 = []
    aggHitC2 = 0
    ...
    for key in group_keys:
        if EvLFU4DLRM_C1.vals_C1.get(key) is not None:
            aggHitMissRecordC1.append(True)
            aggHitC1 += 1
        else:
            aggHitMissRecordC1.append(False)
        if EvLFU4DLRM_C2.vals_C2.get(key) is not None:
            aggHitMissRecordC2.append(True)
            aggHitC2 += 1
        else:
            aggHitMissRecordC2.append(False)
    emb_weights = []
    for key in group_keys:
        val = mixPrecisionUpdate(key, aggHitC1, aggHitC2)
        V = []
        V.append(val)
        V = torch.tensor(V, dtype=torch.float32)
#        print(V)
        emb_weights.append(V)
    if EvLFU4DLRM_C1.lists_C1.get(26) and not len(EvLFU4DLRM_C1.lists_C1.get(26)) == 0:
        EvLFU4DLRM_C1.nPerfectItem_C1 = len(EvLFU4DLRM_C1.lists_C1.get(26))
    if EvLFU4DLRM_C2.lists_C2.get(26) and not len(EvLFU4DLRM_C2.lists_C2.get(26)) == 0:
        EvLFU4DLRM_C2.nPerfectItem_C2 = len(EvLFU4DLRM_C2.lists_C2.get(26))

    aggHitMissRecord = []
    for i in range(26):
        if aggHitMissRecordC1[i] or aggHitMissRecordC2[i]:
...
def mixPrecisionUpdate(key, aggHitC1, aggHitC2):
    # If C1 is Hit, return the val:
    if EvLFU4DLRM_C1.vals_C1.get(key) is not None:
        val = EvLFU4DLRM_C1.getValFromMem(key, aggHitC1)
        return val
    # C1 is Miss, ask C2:
    # C2 Hit:
    if EvLFU4DLRM_C2.vals_C2.get(key) is not None:
        val = EvLFU4DLRM_C2.getValFromMem(key, aggHitC2)
        return val
    # C2 Miss too:
    valC1 = EvLFU4DLRM_C1.getValFromStore(key)
    EvLFU4DLRM_C1.set(key, valC1, aggHitC1)
    valC2 = EvLFU4DLRM_C2.getValFromStore(key)
    EvLFU4DLRM_C2.set(key, valC2, aggHitC2)
    return valC1
...
class DLRM_Net(nn.Module):
    ...
    def create_emb(self, m, ln, weighted_pooling=None):
        emb_l = nn.ModuleList() #list of modules  
        v_W_l = [] #
        for i in range(0, ln.size): #iterate through the size of the ln(what is ln?)
            if ext_dist.my_size > 1: #what is ext_dist
            ...
            if self.qr_flag and n > self.qr_threshold:
                EE = QREmbeddingBag( #embedding bag, quotient remainder trick, defined in qrembedding bag
                    n, #size of dictionary of embeddings
                    m, #size of each embedding vector
                    self.qr_collisions,
                    operation=self.qr_operation,
                    mode="sum", #computes weighted sum
                    sparse=True, #weight matrix will be a sparse tensor
                )
            elif self.md_flag and n > self.md_threshold:
                base = max(m) #find largest of m
    ...
    def apply_emb(self, lS_o, lS_i, emb_l, v_W_l):
        ...
        group_keys = []
        for k, sparse_index in enumerate(lS_i):
            # embedding lookup
...
def inference(
    ...
    if args.mlperf_logging:
        ...
        print ("acc_test: ", acc_test, "test_accu: ", test_accu, "test_samp: ", test_samp)
...
#         for kunci, nilai in ld_model["state_dict"].items():
#             print(kunci)
#             print(type(kunci))
#             print(nilai)
#             print(type(nilai))
...
#         print("Load new set of EV Table from = " + args.ev_path)

#         for ev_idx in range(0, 26):
#             # Reference: Daniar's Github
            
#             # Read new EV Table from file
#             new_ev_path = os.path.join(args.ev_path, "ev-table-" + str(ev_idx + 1) + ".csv")
                        ...
                        if (
                            ...
                            for key,value in model_metrics_dict["state_dict"].items():
                                ...
                                if (key[:3] == "emb"):
                                    ...
                                    out_path = os.path.join("weights_and_biases", "epoch-" + str(k), "ev-table","ev-table-" + str(count+1) + ".csv")
...
#                             with open('weights_and_biases/test.csv', 'w') as f:    
#                                 print("{}".format(",".join(str(x) for x in range(0,dimension))), file=f)
#                                 writer_csv = csv.writer(f)
#                                 for name,param in dlrm.named_parameters():
#                                     if(name[:3] == "emb"):
#                                         writer_csv.writerow(param.detach().cpu().numpy())
#                             for k,v in model_metrics_dict["state_dict"].items():
#                                 arr = numpy.asarray(v)
#                                 numpy.savetxt("weights_and_biases/test.csv", arr, delimiter=",")
                            print("Save to csv")
                            # purposely create an error, so program stops here. cannot do it with sys.exit(-1)
#                             print(curr_ev_table[300])
                        if args.mlperf_logging:
                            mlperf_logger.barrier()
